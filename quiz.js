...
class CyberSecuQuiz {
    ...
    restoreProgress(progress) {
        ...
        this.categoryScores = progress.categoryScores || this.categoryScores;
    ...
    loadQuestion() {
        if (!quizData || !quizData[this.currentQuestion]) {
    ...
    selectAnswer(selectedIndex) {
        if (!quizData || !quizData[this.currentQuestion]) return;
        ...
        if (this.feedbackBox) {
            ...
            this.feedbackBox.classList.remove('correct', 'incorrect');
    ...
    showResults() {
        ...
        const circle = this.scorePercentage ? this.scorePercentage.parentElement : null;
        if (circle) {
            if (percentage >= 80) {
    ...
    generateResultQR(percentage) {
        if (!this.qrPlaceholder) return;
        
        this.qrPlaceholder.innerHTML = '';
        
        // Check if QRCode library is loaded
        if (typeof QRCode === 'undefined') {
            this.qrPlaceholder.innerHTML = '<p>QR Code library not loaded</p>';
            return;
        }
        ...
        qrContainer.id = 'result-qr-code';
        this.qrPlaceholder.appendChild(qrContainer);
        ...
        try {
            ...
            this.qrPlaceholder.appendChild(downloadBtn);
            
        } catch (e) {
            console.error('QR generation failed:', e);
            this.qrPlaceholder.innerHTML = '<p>Could not generate QR code</p>';
        }
    }
